class FilesController < ApplicationController
  #Creamos dos constantes, para que el resto de los métodos puedan usarlas. Una será la ruta al archivo de comentarios y la otra la ruta al directorio donde se guardarán los archivos .doc, .docx y .pdf.
  route_file_comment = "public/comments/comment.txt";
  route_file_dirfiles = "public/files";
################################################################################################
  def upload_files
    # Debería permitir mostrar un formulario de subida de archivos y la acción para subirlos, así que a la misma también debería accederse por POST.
    get "files/upload_files"
    post "archivos/upload_files"
...
################################################################################################
  def list_files
    # No se tocará, sólo por get está bien.
    get "files/list_files"
    
    Guardamos la lista de archivos de la carpeta "files".
...
#############################################################################################
  def destroy_files
    # No se accederá por GET, sino por POST, ya que sólo servirá para borrar archivos.
    post "files/destroy_files"
...
################################################################################################
  def comment_files
    # Deberá tener la posibilidad de accederse por POST, ya que por GET nos mostrará un fomulario y por POST recibirá los datos de ese formulario.
    get "files/comment_files"
    post "files/comment_files"
